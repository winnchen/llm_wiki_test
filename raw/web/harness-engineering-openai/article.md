...
![Diagram: Codex drives the app with Chrome DevTools MCP to validate its work (snapshot → fix → loop).](assets/fig-01-codex-chrome-devtools.png)
...
![Diagram: Giving Codex a full observability stack in local dev (logs/metrics/traces → query → fix loop).](assets/fig-02-observability-stack.svg)
...
![Diagram: The limits of agent knowledge — what Codex can’t see doesn’t exist; encode into the repo.](assets/fig-03-agent-knowledge-limits.png)
...
![Layered domain architecture with explicit cross-cutting boundaries (Types → … → UI; Providers).](assets/fig-04-layered-architecture.png)
...
> 离线配图：正文中的四幅示意图已落盘至 `assets/`（源站资源托管在 Contentful `images.ctfassets.net`）；OpenAI 主站对自动化抓取常返回 403，更新配图时宜用浏览器或只读代理拉取同 URL。
