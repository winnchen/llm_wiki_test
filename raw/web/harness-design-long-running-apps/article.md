...
![Illustration from the original article (offline copy).](assets/fig-01.png)
...
![Illustration from the original article (offline copy).](assets/fig-02.png)
...
![Illustration from the original article (offline copy).](assets/fig-03.png)
...
![Illustration from the original article (offline copy).](assets/fig-04.png)
...
![Illustration from the original article (offline copy).](assets/fig-05.png)
...
![Solo harness: opening screen, sprite editor, and gameplay (screenshots from the original post).](assets/fig-06.png)
...
![Full harness: opening, editors, AI-assisted design, and gameplay (screenshots from the original post).](assets/fig-07.png)
...
> 原文内嵌演示视频；离线归档未收录视频文件（亦无单独封面静帧 URL）。
...
![Long-form example / artifact screenshot from the original post (offline copy).](assets/fig-08.png)
...
> 离线配图：`assets/fig-01.png`–`fig-08.png` 自 `www-cdn.anthropic.com` 按页面出现顺序拉取；`fig-01`–`fig-05` 在正文中仅标为「Illustration from the original article」——若需与原文逐段对齐，请对照 [Anthropic 原文](https://www.anthropic.com/engineering/harness-design-long-running-apps)。`fig-06`/`fig-07` 对应 solo / full harness 界面拼图说明。
