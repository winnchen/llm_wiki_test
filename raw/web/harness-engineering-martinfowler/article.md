...
![Figure 1: The term "harness" means different things depending on the bounded context.](assets/harness-bounded-contexts.png)
...
![Overview of guides (feedforward) and feedback sensors around a coding agent; human steers both.](assets/harness-overview.png)
...
| Area | Direction | Computational / Inferential | Example implementations |
| --- | --- | --- | --- |
...
![Examples of feedforward and feedback in a change’s lifecycle (local loop, human review, pipeline).](assets/harness-change-lifecycle-examples.png)
...
![Examples of continuous drift and runtime feedback sensors outside the single-change lifecycle.](assets/harness-continuous-feedback-examples.png)
...
![Harness types: maintainability, architecture fitness, and behaviour (with example guides/sensors).](assets/harness-types.png)
...
![Harness templates as bundles of guides/sensors per service topology (concept sketch).](assets/harness-templates.png)
...
> 离线归档；正文配图已同步至同目录 `assets/`（与 [原文](https://martinfowler.com/articles/harness-engineering.html) 对照）；站内侧栏-only 内容仍以原文为准。
